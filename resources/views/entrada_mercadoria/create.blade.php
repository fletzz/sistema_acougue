<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div style="margin-bottom: 20px;">
                <a href="{{ route('entrada_mercadoria.index') }}" style="margin-right: 10px; color: #2563eb; text-decoration: underline;">← Voltar para Entradas</a>
            ...
            <form method="POST" action="{{ route('entrada_mercadoria.store') }}" id="formEntrada">
                ...
                <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                    <h3 style="font-size: 18px; font-weight: bold; margin-bottom: 15px;">Informações da Entrada</h3>
                    
                    <div style="margin-bottom: 15px;">
                        <label>Fornecedor (Opcional):</label>
                        <select name="fornecedor_id" style="width: 100%; padding: 8px;">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="date" name="data_entrada" value="{{ date('Y-m-d') }}" required style="width: 100%; padding: 8px;">
                    </div>
                ...
                <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                    <h3 style="font-size: 18px; font-weight: bold; margin-bottom: 15px;">Bovinos que Chegaram (Matéria-Prima)</h3>
                    <p style="color: #6b7280; font-size: 14px; margin-bottom: 15px;">Use esta tela para registrar entrada de bovinos brutos (traseiro, dianteiro, etc) que serão transformados depois.</p>
                    
                    <div id="itens-container">
                        <div class="item-row" style="margin-bottom: 15px; padding: 15px; border: 1px solid #e5e7eb; border-radius: 8px;">
                            <div style="margin-bottom: 10px;">
                                <label>Produto:</label>
                                <select name="produto_id[]" class="produto-entrada" required style="width: 100%; padding: 8px;">
                                    <option value="">Selecione um produto existente</option>
                                    <option value="__NOVO__">➕ Cadastrar Produto Novo</option>
                            ...
                            <div class="campos-produto-existente" style="display: flex; gap: 10px; align-items: end;">
                                <div style="flex: 1;">
                                    ...
                                    <input type="number" name="quantidade[]" class="quantidade-entrada" step="0.001" min="0.001" required style="width: 100%; padding: 8px;">
                                </div>
                                <div style="flex: 1;">
                                    <label>Preço Custo (R$):</label>
                                    <input type="number" name="preco_custo[]" class="preco-custo" step="0.01" min="0" style="width: 100%; padding: 8px;">
                                </div>
                            </div>
                            ...
                            <div class="campos-produto-novo" style="display: none; margin-top: 10px; padding: 15px; background: #f3f4f6; border-radius: 6px;">
                                <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                                    <div>
                                        <label>Nome do Bovino (ex: Traseiro, Dianteiro, Boi Inteiro):</label>
                                        <input type="text" name="nome_produto[]" class="nome-produto-novo" placeholder="Ex: Traseiro Bovino" style="width: 100%; padding: 8px;">
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="unidade_medida[]" class="unidade-produto-novo" placeholder="KG" value="KG" style="width: 100%; padding: 8px;">
                                    </div>
                                </div>
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                                    <div>
                                        <label>Quantidade (Peso em KG):</label>
                                        <input type="number" name="quantidade[]" class="quantidade-entrada-novo" step="0.001" min="0.001" style="width: 100%; padding: 8px;">
                                    </div>
                                    <div>
                                        <label>Preço Custo (R$):</label>
                                        <input type="number" name="preco_custo[]" class="preco-custo-novo" step="0.01" min="0" style="width: 100%; padding: 8px;">
                                    </div>
                                </div>
                                <div style="background: #dbeafe; padding: 10px; border-radius: 4px; margin-top: 10px;">
                                    <small style="color: #1e40af;">ℹ️ Este bovino será transformado depois. O preço de venda será definido nos cortes finais.</small>
                                </div>
                            </div>
                ...
                <button type="submit" style="padding: 12px 30px; background: #2563eb; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 16px;">Registrar Entrada</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('formEntrada').addEventListener('submit', function(e) {
            const itens = [];
            document.querySelectorAll('.item-row').forEach(row => {
                const produtoSelect = row.querySelector('.produto-entrada');
                ...
                if (produtoId === '__NOVO__') {
                    ...
                    const quantidade = row.querySelector('.quantidade-entrada-novo').value;
                    const precoCusto = row.querySelector('.preco-custo-novo').value;
                    ...
                    const quantidade = row.querySelector('.quantidade-entrada').value;
                    const precoCusto = row.querySelector('.preco-custo').value;
